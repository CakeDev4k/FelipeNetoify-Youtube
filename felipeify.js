...
function getTweetAndChange(){
  tweetClass = 'article[data-testid="tweet"]'
  tweet = document.querySelectorAll(tweetClass)

  felipeClass = "a[href='/felipeneto'] > div > span"
  felipe = document.querySelectorAll(felipeClass)

  tweetsTextClass = 'div[data-testid="tweetText"] > span:not(#edited)'
  tweetText = document.querySelectorAll(tweetsTextClass)

  imgClass = 'div[data-testid="tweetPhoto"] > div:not(#edited)'
  imgTweet = document.querySelectorAll(imgClass)

  imgTweet.forEach((imgs) => {
    imgs.style.backgroundImage = `url(${listTweets.photos[Math.floor(Math.random() * listTweets.photos.length)]})`
    imgs.setAttribute("id", "edited")
  })

  tweetText.forEach((tweets) => {
        let change = Math.random() < 0.10;
        if(!change){
          tweets.innerText = "Felipe neto "
        }else{
          tweets.innerText = "Nelipe feto "
        }

        felipe.forEach(() => {
          tweets.innerText = listTweets.tweetFelipe[Math.floor(Math.random() * listTweets.tweetFelipe.length)]
        })
        
        tweets.setAttribute("id", "edited")
  })
}

function GetUrlTweetFelipe() {
  fetch(chrome.runtime.getURL(`tweets/Tweets.json`))
    .then(response => response.json())
    .then(data => {
      listTweets = data
    })
    .catch((error) => {
      console.log(error + " sem imagem")
      return
    });
}
